...
class ViewVehicalCard extends StatefulWidget {
 String img,com,model,price,capacity,type,name,status;

 ViewVehicalCard(
      this.img, this.com, this.model, this.price, this.capacity, this.type,this.name,this.status);

  @override
  State<ViewVehicalCard> createState() => _ViewVehicalCardState();
}

class _ViewVehicalCardState extends State<ViewVehicalCard> {
  @override
            ...
            GestureDetector(
              onTap: (){
                if(widget.status=="do"){
                  ...
                  Navigator.push(context, MaterialPageRoute(builder:(BuildContext)=>UserBookScreen(widget.img, widget.com, widget.model, widget.price, widget.capacity, widget.type,widget.name)));
                }
